...
- (void)internalSeekToTime:(double)newSeekTime
{
		...
		calculatedBitRate > 0)
	{
		...
		if (!err && !(ioFlags & kAudioFileStreamSeekFlag_OffsetIsEstimated))
		{
			...
			seekByteOffset = (NSInteger)(packetAlignedByteOffset + dataOffset);//lilin
		}
	}
	...
	ioFlags:(UInt32 *)ioFlags
{
	@synchronized(self)
	{
		...
		else if (inPropertyID == kAudioFileStreamProperty_DataOffset)
		{
			...
			dataOffset = (NSInteger)offset;//lilin
			
			if (audioDataByteCount)
			{
				fileLength = (NSInteger)(dataOffset + audioDataByteCount);//lilin
			}
		}
	...
	if (inPacketDescriptions)
	{
		for (int i = 0; i < inNumberPackets; ++i)
		{
			...
			@synchronized(self)
			{
				...
				AudioQueueBufferRef fillBuf = audioQueueBuffer[fillBufferIndex];
				memcpy((char*)fillBuf->mAudioData + bytesFilled, (const char*)inInputData + packetOffset,(unsigned long)packetSize);//lilin
